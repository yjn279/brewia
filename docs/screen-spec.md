...
flowchart LR
    ...
    Login -->|signIn 成功| Home
    Home -->|Add bean| New
    Home -->|Bean カード| BeanDetail
    Home -->|Presets ボタン| Presets
    BeanDetail -->|Edit| BeanEdit
    BeanDetail -->|Add brew| New
    BeanDetail -->|Copy bean| New
    BeanDetail -->|Brew カード| BrewDetail
    BeanEdit -->|Save| BeanDetail
    BrewDetail -->|Edit| BrewEdit
    BrewDetail -->|Copy brew| New
    BrewDetail -->|Bean リンク| BeanDetail
    BrewEdit -->|Save| BrewDetail
    Presets -->|Edit| Presets
    New -->|Save bean| BeanDetail
    New -->|Save brew| BrewDetail
    Home -.->|未認証| Login
    Home -.->|オフライン| Offline
```
...
- 1 つのエントリポイントで 「豆 (Bean) の追加」と「抽出 (Brew) のログ」を切替可能にする

### 4.2 タブ切替（`NewEntryTabs`）
...
- 豆 1 件のメタ情報と、その豆で淹れた抽出履歴を一画面で見せる
...
- 1 件の抽出ログを、参照豆・パラメータ・注湯プロファイル・テイスティングを一画面で見せる
