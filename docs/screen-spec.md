...
- ヘッダ: 左に戻るボタン / タイトル、右にアクション群と `UserMenu`
...
| 操作 | 結果 |
| --- | --- |
| Edit | `PresetEditDialog`（抽出ステップ UI は New Brew Form と同形） |
| Delete | 確認後 `DELETE /api/brew-presets/{id}` |
| Start brewing | `/new?type=brew` |

固定 / 組み込みプリセットは表示しない。プリセット保存ダイアログは `New Brew Form` の `Extraction Steps` ヘッダから起動する。
