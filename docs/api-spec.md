...
指定した ID の豆を取得する。豆詳細ページ（`/beans/{id}`）や編集ページで呼び出される。他ユーザーの豆は 404 を返す。
...
指定した ID の豆を更新する。豆編集フォームの保存時に呼び出される。他ユーザーの豆は 404 を返す。
...
指定した ID の豆を削除する。豆詳細ページの削除ボタンから呼び出される。他ユーザーの豆は 404 を返す。
...
自分の抽出一覧を返す。`beanId` クエリを指定すると特定の豆に絞り込んだ結果を返す。豆詳細ページの抽出履歴で呼び出される。
...
抽出ログを新規作成する。抽出フォームの保存時に呼び出される。`flavorIds` は重複除去された上で `brew_flavor` に挿入される。
...
指定した ID の抽出を豆情報とフレーバー一覧を結合して返す。抽出詳細ページで呼び出される。他ユーザーの抽出は 404 を返す。
...
指定した ID の抽出を更新する。抽出編集フォームの保存時に呼び出される。`PUT /api/brews/:id` のレイヤ間処理は「データフロー」セクションのシーケンス図を参照。
...
指定した ID の抽出を削除する。抽出詳細ページの削除ボタンから呼び出される。紐づく `brew_flavor` も削除される。
...
ユーザーのプリセット一覧を返す。固定 / 組み込みプリセットは存在せず、ユーザーが作成したもののみ返す。`/presets` 画面で呼び出される。

### POST /api/brew-presets

プリセットを新規作成する。抽出フォームの「Extraction Steps」ヘッダから起動するプリセット保存ダイアログで呼び出される。
...
指定した ID のプリセットを返す。プリセット編集ダイアログの初期データ読み込みで呼び出される。他ユーザーのプリセットは 404 を返す。
...
指定した ID のプリセットを更新する。プリセット編集ダイアログの保存時に呼び出される。`steps` 配列は最低 1 件必要。

### DELETE /api/brew-presets/{id}

指定した ID のプリセットを削除する。プリセット一覧の削除ボタンから呼び出される。

## Flavors API

### GET /api/flavors

全ユーザー共通のフレーバーマスタを返す。抽出フォームのフレーバー選択 UI で呼び出される。
...
Auth.js が管理する認証エンドポイント群。アプリコードからは直接呼び出さず、Auth.js がサインイン / サインアウト / コールバック処理を担う。次のサブパスを受け持つ。
