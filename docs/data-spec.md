...
`brew_flavor` に `user_id` を冗長に保持しない理由: `brew` が必ず `user_id` を持ち、JOIN で一意に決まる。中間テーブルに重複させるとアクセス制御の不整合バグの温床になる。
...
更新時は対象 `brew_id` の行を全削除してから再挿入する（差分更新ではない）。
...
`process` のみ DB 上の制約は単なる `text NOT NULL`。バリデーションは Zod スキーマと UI 側 select で担保する。
