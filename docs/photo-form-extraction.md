...
- `fetch` が失敗・レスポンスが `ok: false` のいずれの場合も、`toast.error('自動入力に失敗しました。手動で入力してください')` を表示する
- フォームは一切変更しない（部分的に入力済みの値も維持される）
- `isLoading` を `false` に戻してボタンを再度有効化する
- ユーザーはそのまま手動で入力を続けられる
