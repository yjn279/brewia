...
import { createBrew } from '@/features/brews/api'
...
export default function NewBrewScreen() {
  ...
  async function onSubmit(data: UpsertBrewDto) {
    ...
    try {
      await createBrew(session.user.id, data)
      router.back()
    } catch (err) {
  ...
  return (
    <ScreenContainer>
      <Text style={styles.heading}>New Brew</Text>
      ...
      <Controller
        ...
        name="aroma"
        render={({ field: { value, onChange } }) => (
          <RatingField label="Aroma" value={value} onChange={onChange} />
        )}
      />
      <Controller
        control={control}
        name="acidity"
        render={({ field: { value, onChange } }) => (
          <RatingField label="Acidity" value={value} onChange={onChange} />
        )}
      />
      <Controller
        control={control}
        name="sweetness"
        render={({ field: { value, onChange } }) => (
          <RatingField label="Sweetness" value={value} onChange={onChange} />
        )}
      />
      <Controller
        control={control}
        name="body"
        render={({ field: { value, onChange } }) => (
          <RatingField label="Body" value={value} onChange={onChange} />
        )}
      />
      <Controller
        control={control}
        name="overall"
        render={({ field: { value, onChange } }) => (
          <RatingField label="Overall" value={value} onChange={onChange} />
        )}
      />
