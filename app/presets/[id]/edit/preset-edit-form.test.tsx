...
describe('PresetEditForm', () => {
  ...
  it('renders all form fields with initial preset values', () => {
    ...
    expect((screen.getByLabelText('Water Weight (g)') as HTMLInputElement).value).toBe('300')
  ...
  it('shows empty string for defaultWaterWeight when value is 0', () => {
    const preset = { ...samplePreset, defaultWaterWeight: 0 }
    render(<PresetEditForm preset={preset} />)

    expect((screen.getByLabelText('Water Weight (g)') as HTMLInputElement).value).toBe('')
  })
  ...
  it('calls PUT /api/brew-presets/:id with correct body on Save', async () => {
    ...
    expect(body.defaultWaterWeight).toBe(300)
