...
describe('BrewTimer', () => {
  ...
  // confirm dialog: stopped で Reset 押下では onReset が呼ばれず、AlertDialog が開く
  it('confirm dialog: clicking Reset in stopped state does NOT call onReset and opens the AlertDialog', () => {
    const onReset = vi.fn()
    const noop = vi.fn()
    render(
      <BrewTimer
    ...
    expect(
      screen.getByText('リセットするとタイマーと抽出ステップが初期化されます。続行しますか？'),
    ).toBeDefined()
  })

  // confirm dialog: ダイアログの「リセット」ボタン押下で onReset がちょうど 1 回
  it('confirm dialog: clicking the confirm "リセット" button calls onReset exactly once', () => {
    const onReset = vi.fn()
    const noop = vi.fn()
    render(
      <BrewTimer
  ...
  // confirm dialog: ダイアログの「キャンセル」ボタン押下で onReset は呼ばれない
  it('confirm dialog: clicking "キャンセル" does not call onReset', () => {
    const onReset = vi.fn()
    const noop = vi.fn()
    render(
      <BrewTimer
